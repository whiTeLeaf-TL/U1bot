...
async def _(event: GroupMessageEvent, matcher: Matcher):
    if __op_type := matcher.get_arg("op_type", None):
        _op_type = __op_type.extract_plain_text()
    else:
        _op_type = ""
    if __item := matcher.get_arg("item", None):
        _item = __item.extract_plain_text()
    else:
        _item = ""
    if __param1 := matcher.get_arg("param1", None):
        _param1 = int(__param1.extract_plain_text())
    else:
        _param1 = 0

    if __param2 := matcher.get_arg("param2", None):
