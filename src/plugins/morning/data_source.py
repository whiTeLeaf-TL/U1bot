...
class MorningManager:
    ...
    def get_group_config(self) -> MessageSegment:
        ...
        if morning_intime:
            msg += (
                "是\n - 最早允许起床时间："
                + str(self._config["morning"]["morning_intime"]["early_time"])
                + "点\n - 最晚允许起床时间："
                + str(self._config["morning"]["morning_intime"]["late_time"])
                + "点"
            )
        else:
            msg += "否"
        ...
        if multi_get_up:
            ...
            msg += (
                "否\n - 允许的最短起床间隔："
                + str(self._config["morning"]["multi_get_up"]["interval"])
                + "小时"
            )
        ...
        if super_get_up:
            ...
            msg += (
                "否\n - 允许的最短睡觉时长："
                + str(self._config["morning"]["super_get_up"]["interval"])
                + "小时"
            )
        ...
        if night_intime:
            msg += (
                "是\n - 最早允许睡觉时间："
                + str(self._config["night"]["night_intime"]["early_time"])
                + "点\n - 最晚允许睡觉时间：第二天早上"
                + str(self._config["night"]["night_intime"]["late_time"])
                + "点"
            )
        else:
            msg += "否"
        ...
        if good_sleep:
            ...
            msg += (
                "否\n - 允许的最短优质睡眠："
                + str(self._config["night"]["good_sleep"]["interval"])
                + "小时"
            )
        ...
        if deep_sleep:
            ...
            msg += (
                "否\n - 允许的最短清醒时长："
                + str(self._config["night"]["deep_sleep"]["interval"])
                + "小时"
            )
    ...
    def _change_set_time(
        self,
        _day_or_night: str,
        _setting: str,
        _interval_or_early_time: int,
        _late_time: Optional[int] = None,
    ) -> str:
        """
        ...
        if _setting in {"morning_intime", "night_intime"}:
        ...
        if (
            _setting == "morning_intime"
            or _setting == "night_intime"
            or _setting == "good_sleep"
        ...
        if (
            _setting == "multi_get_up"
            or _setting == "super_get_up"
            or _setting == "deep_sleep"
    ...
    def morning_config(
        self, _mor_setting: str, param1: int, param2: int
    ) -> MessageSegment:
        """
    ...
    def night_config(
        self, _nig_setting: str, param1: int, param2: int
    ) -> MessageSegment:
        """
    ...
    # ------------------------------ Morning Judgement ------------------------------ #
    def _morning_and_update(
        self, gid: str, uid: str, now_time: datetime
        ...
        if in_sleep.days > 0:
            ...
            self._morning[gid][uid]["weekly"]["weekly_sleep"] = sleeptime_update(
                self._morning[gid][uid]["weekly"]["weekly_sleep"], in_sleep
            )
            self._morning[gid][uid]["total"]["total_sleep"] = sleeptime_update(
        ...
        return (
            self._morning[gid]["group_count"]["daily"]["good_morning"],
            in_sleep_tmp if in_sleep_tmp != "" else 0,
        )
    ...
    def get_morning_msg(self, gid: str, uid: str, sex_str: str) -> MessageSegment:
        ...
        if uid in self._morning[gid]:
            ...
            if last_sleep_time - now_time < timedelta(hours=24):
                ...
                if (
                    not self._config["morning"]["multi_get_up"]["enable"]
                    and self._morning[gid][uid]["daily"]["morning_time"] != 0
                ):
                ...
                if not self._config["morning"]["super_get_up"]["enable"]:
                    ...
                    night_time: datetime = datetime.strptime(
                        self._morning[gid][uid]["daily"]["night_time"],
                        "%Y-%m-%d %H:%M:%S",
                    )
    ...
    def _night_and_update(
        ...
        if uid not in self._morning[gid]:
            self._morning[gid].update(
                {
                    uid: {
                        "daily": {"morning_time": 0, "night_time": now_time},
                        "weekly": {
        ...
        # 若有就更新数据
            ...
            lastweek_lnt: Union[int, str] = self._morning[gid][uid]["weekly"][
                "lastweek_latest_night_time"
            ]
            ...
            if isinstance(lastweek_lnt, int):
                ...
                if is_later(
                    self._morning[gid][uid]["daily"]["night_time"],
                    self._morning[gid][uid]["weekly"]["lastweek_latest_night_time"],
                ):
                    self._morning[gid][uid]["weekly"][
                        "lastweek_latest_night_time"
                    ] = now_time
        ...
        if self._morning[gid][uid]["daily"].get("morning_time", 0) != 0:
            get_up_time: datetime = datetime.strptime(
                self._morning[gid][uid]["daily"]["morning_time"], "%Y-%m-%d %H:%M:%S"
            )
            ...
            in_day_tmp = "" if in_day.days > 0 else f"{hours}时{minutes}分{seconds}秒"
        # 判断是今天第几个睡觉的
        ...
        return (
            self._morning[gid]["group_count"]["daily"]["good_night"],
            in_day_tmp if in_day_tmp != "" else 0,
        )
        ...
        if uid in self._morning[gid]:
            ...
            if today == MONDAY:
                ...
                if hour != -1 and is_later_oclock(now_time, hour):
                    lastweek_morning_count: int = self._morning[gid][uid]["weekly"][
                        "lastweek_morning_count"
                    ]
                    lastweek_night_count: int = self._morning[gid][uid]["weekly"][
                        "lastweek_night_count"
                    ]
                    lastweek_sleep: List[int] = self._morning[gid][uid]["weekly"][
                        "lastweek_sleep"
                    ]
            ...
            # Not on Monday, add weekly info
                ...
                weekly_morning_count: int = self._morning[gid][uid]["weekly"][
                    "weekly_morning_count"
                ]
                weekly_night_count: int = self._morning[gid][uid]["weekly"][
                    "weekly_night_count"
                ]
    ...
    def _save_data(self) -> None:
        with open(self._morning_path, "w", encoding="utf-8") as f:
            json.dump(
                self._morning, f, ensure_ascii=False, indent=4, cls=DateTimeEncoder
            )
    ...
    def get_refresh_time(self, _time: str, key: str) -> int:
        ...
        return (
            self._config[_time][f"{_time}_intime"][key]
            if self._config[_time][f"{_time}_intime"]["enable"]
            else -1
        )
    ...
    def weekly_night_refresh(self) -> None:
        ...
        for gid in self._morning:
            for uid, user_items in self._morning[gid].items():
                ...
                user_items["weekly"]["lastweek_night_count"] = user_items["weekly"][
                    "weekly_night_count"
                ]
    ...
    def weekly_sleep_time_refresh(self) -> None:
        ...
        for gid in self._morning:
            ...
            for uid, user_items in self._morning[gid].items():
                ...
                user_items["weekly"]["lastweek_morning_count"] = user_items["weekly"][
                    "weekly_morning_count"
                ]
                user_items["weekly"]["lastweek_sleep"] = user_items["weekly"][
                    "weekly_sleep"
                ]
            ...
            self._morning[gid]["group_count"]["weekly"][
                "sleeping_king"
            ] = _sleeping_king_uid
