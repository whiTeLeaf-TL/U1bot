...
pic2_text = on_command("字符画", priority=26, block=True)
...
@pic2_text.got("image", prompt="啊？图呢？你再发个图出来我看看试试？")
async def generate_(state: T_State):
    msg = state["image"]
    if msg[0].type == "image":
        url = msg[0].data["url"]  # 图片链接
        await pic2_text.send("让姚奕思考一下...")

        pic = await get_img(url)  # 取图
        if not pic:
            await pic2_text.finish("图片因为靠近”错误“黑洞而消失了")

        if pic.format == "GIF":
            res = await char_gif(pic)
            await pic2_text.finish(MessageSegment.image(res))
        text = await get_pic_text(pic)
        if text:
            res = await text2img(text)
            await pic2_text.finish(MessageSegment.image(res))
    else:
        await pic2_text.finish("气死我力！要的是图！")
