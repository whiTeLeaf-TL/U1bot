...
async def music_set_rule(event: Union[GroupMessageEvent, PrivateMessageEvent]) -> bool:
    # 权限设置
    return (
        event.sender.role in ADMIN[: ncm_config.ncm_admin_level]
        or event.get_user_id() in ncm_config.superusers
    )
        ...
        if len(args) == 1:
            ...
            if isinstance(event, GroupMessageEvent):
                ...
                if info:
                    ...
                    if mold in TRUE:
                        setting.insert(
                            {"user_id": event.user_id, "song": True, "list": True}
                        )
                    elif mold in FALSE:
                        setting.insert(
                            {"user_id": event.user_id, "song": False, "list": False}
                        )
