...
class Ncm:
    ...
    def get_session(
        self, bot: Bot, event: Union[GroupMessageEvent, PrivateMessageEvent]
    ):
        self.bot = bot
        self.event = event
    ...
    async def music_check(self, nid):
        nid = int(nid)
        if not (info := music.search(Q["id"] == nid)):
            return None
        path = Path(info[0]["file"])
        return info[0] if path.is_file() else await self.download(ids=[nid], check=True)
    ...
    async def search_song(self, keyword: str, limit: int = 1) -> int:  # 搜索歌曲
        ...
        data = res["result"]["songs"] if "result" in res.keys() else res["songs"]
        if data:
            return data[0]["id"]
    ...
    def check_message(self):
        ...
        :return:
        """
        info = ncm_check_cache.search(
            Q.message_id == self.event.dict()["reply"]["message_id"]
        )
    ...
    def get_song(self, nid: Union[int, str], message_id=None):
        ...
        :return:
        """
        mid = message_id or self.event.message_id
    ...
    async def upload_group_file(self, file, name):
        try:
            await self.bot.upload_group_file(
                group_id=self.event.group_id, file=file, name=name
            )
        except (ActionFailed, NetworkError) as e:
            logger.error(e)
            if (
                isinstance(e, ActionFailed)
                and e.info["wording"] == "server" " requires unsupported ftn upload"
            ):
                await self.bot.send(
                    event=self.event,
                    message=Message(
    ...
    async def upload_private_file(self, file, name):
        try:
            await self.bot.upload_private_file(
                user_id=self.event.user_id, file=file, name=name
            )
        except (ActionFailed, NetworkError) as e:
            logger.error(e)
    ...
    def get_zip(self, lid, filenames: list):
        ...
        with zipfile.ZipFile(
            str(Path.cwd().joinpath("music").joinpath(zip_file_new)),
            "w",
    ...
    async def download(self, ids: list, check=False, lid=0, is_zip=False):  # 下载音乐
        data: list = self.api.track.GetTrackAudio(song_ids=ids, bitrate=3200 * 1000)[
            "data"
        ]
