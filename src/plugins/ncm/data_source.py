...
if not musicPath.is_dir():
    musicPath.mkdir()
    logger.success("ncm 音乐库创建成功")
if not dbPath.is_dir():
    dbPath.mkdir()
    logger.success("ncm 数据库目录创建成功")
...
class Ncm:
    ...
    def get_qrcode(self):
        ...
        img.save("ncm.png")
        logger.info(
            "二维码已经保存在当前目录下的 ncm.png，请使用手机网易云客户端扫码登录。"
        )
        while True:
    ...
    def get_detail(self, ids: List[int]):
        data: list = self.api.track.GetTrackAudio(
            song_ids=ids, bitrate=ncm_config.ncm_bitrate * 1000
        )["data"]
    ...
    async def upload_data_file(
        ...
        if isinstance(event, GroupMessageEvent):
            await self.upload_group_file(
                group_id=event.group_id, file=data["file"], name=data["filename"]
            )
        elif isinstance(event, PrivateMessageEvent):
            await self.upload_private_file(
                user_id=event.user_id, file=data["file"], name=data["filename"]
            )
    ...
    async def upload(
        self, data: dict, fr: str, event: Union[GroupMessageEvent, PrivateMessageEvent]
    ):
        if data["code"] == 404:
    ...
    async def start_upload(
        self, ids: List[int], event: Union[GroupMessageEvent, PrivateMessageEvent]
    ):
        """一般地 320k 及以上即 flac, 320k 及以下即 mp3,96k 及以下即 m4a"""
