...
class Scheduler:
    ...
    async def run_task(self, task_time: Optional[time] = None):
        """执行定时任务
        ...
        """
        async with get_session() as session:
            statement = select(Schedule).where(Schedule.time == task_time)
            results = await session.scalars(statement)
            schedules = results.all()
            # 如果该时间没有需要执行的定时任务，且不是默认任务则从任务列表中删除该任务
            if task_time and not schedules:
                self.schedules.pop(task_time.isoformat()).remove()
                return
            logger.info(f"开始发送每日词云，时间为 {task_time or '默认时间'}")
