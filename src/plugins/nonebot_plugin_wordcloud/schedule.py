...
class Scheduler:
    ...
    async def run_task(self, time_param: Optional[time] = None):
        """执行定时任务
        ...
        """
        async with get_session() as session:
            statement = select(Schedule).where(Schedule.time == time_param)
            results = await session.scalars(statement)
            schedules = results.all()
            # 如果该时间没有需要执行的定时任务，且不是默认任务则从任务列表中删除该任务
            if time_param and not schedules:
                self.schedules.pop(time_param.isoformat()).remove()
                return
            logger.info(f"开始发送每日词云，时间为 {time_param or '默认时间'}")
