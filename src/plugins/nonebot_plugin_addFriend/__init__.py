...
againReadConfig = on_command(
    "重载配置",
    aliases={"更改自动同意", "更改最大加好友数量", "更改查看加返回数量", "更改加好友时间单位"},
    priority=5,
    block=True,
...
async def _(bot: Bot, event: MessageEvent, args: Message = CommandArg()):
    ...
    try:
        if autoType == "group":
            resMsg = (
                "群号{}，邀请者{}".format(QQOrGroupId, requestorId)
                + notice_msg
                + comment
                + "\n时间:{}\n".format(time)
            )
...
async def sendPrivate(bot: Bot, event: PrivateMessageEvent):
    ...
    if event.get_user_id() != config["recipientList"][0]:
        ...
        await bot.send_private_msg(
            user_id=int(config["recipientList"][0]),
            message="叮~私聊消息\nqq:{}\n昵称:{}\n消息:{}".format(
                event.user_id, event.sender.nickname, plaintext
            ),
            auto_escape=False,
        )
