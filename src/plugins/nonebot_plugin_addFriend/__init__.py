...
async def _(bot: Bot, event: RequestEvent):
    ...
    agreeAutoApprove, status = isNormalAdd(
        config[bot.self_id], autoType, addInfo, agreeAutoApprove
    )  # 正常添加判断，过滤无意义添加，类似 xx 通知群
    ...
    # 验证信息
    if not filterFriend(
        event.comment, autoType, config[bot.self_id]["allowAddFriednText"]
    ):
        status = "\n未通过验证消息，已拒绝"
        # 不记录
        await sendMsg(bot, config[bot.self_id]["recipientList"], msg + status, 0)
        ...
        return
    num = parseTime(
        config[bot.self_id]["numControl"], numDict[bot.self_id][autoType], now
    )
    if agreeAutoApprove == 0 or num == -1:
        if num == -1:
            ...
            status = "\n未允许自动添加"
        requestorDict[bot.self_id][autoType][id] = {
            "flag": event.flag,
            "comment": event.comment,
            "notice_msg": notice_msg,
            "staus": status,
            "requestorId": event.user_id,
            "time": time,
        }
...
def handle_time_unit_command(bot, argsText, autoType):
    ...
    return (
        f'更改成功，为{configUtil.config[bot.self_id]["numControl"][autoType]["unit"]}'
    )
...
def handle_max_time_command(bot, argsText, autoType):
    ...
    return (
        f'更改成功，为{configUtil.config[bot.self_id]["numControl"][autoType]["time"]}'
    )
        ...
        if auto_type == "group":
            ...
            if int(qq_or_group_id) in group_list:
                ...
                await bot.set_group_add_request(
                    flag=flag, sub_type="add", approve=approve
                )
        else:
...
async def _(bot: Bot, args: Message = CommandArg()):
    ...
    if (
        parseTime(
            config[bot.self_id]["numControl"][autoType],
            numDict[bot.self_id][autoType],
            now,
        )
        != -1
    ):
        await reFriendReqNum.send(
            message=f'未增满{maxnum}人，人数为{numDict[bot.self_id][autoType]["count"]}上次添加时间{now}'
        )
    ...
    writeTime(numDictPath, numDict)
    await reFriendReqNum.finish(
        f'重置成功，为{numDict[bot.self_id][autoType]["count"]}'
    )
...
async def _(bot: Bot, event: MessageEvent, args: Message = CommandArg()):
    ...
    if int(recipient) in friend_list:
        ...
        await addRecipient.finish(
            f'不是{config[bot.self_id]["botName"]}的好友或者格式错误'
        )
